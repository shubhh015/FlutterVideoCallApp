...
import 'package:flutter_video_call_app/core/repos/token_service.dart';
...
class _StreamDogFoodingAppContentState
  ...
  Future<void> _consumeIncomingCall() async {
    ...
    final userId =
        calls.first.uuid; // Assuming this is where the user ID is stored
    final userSnapshot =
        await FirebaseDatabase.instance.ref('users/$userId').get();

    UserInfo user = UserInfo(id: "1");
    if (userSnapshot.exists && userSnapshot.value != null) {
      final entry = Map<String, dynamic>.from(
          userSnapshot.value as Map<dynamic, dynamic>);

      // Constructing UserInfo from the retrieved data
      user = UserInfo(
        image: entry['image'] ?? "",
        role: entry['role'] ?? "admin",
        name: entry['name'] ?? "John",
        id: entry['id'] ?? "1",
      );
    } else {
    ...
    callResult.fold(success: (result) async {
      ...
      _router.push(CallRoute($extra: extra, user: user).location, extra: extra);
    }, failure: (error) {
  ...
  Future<void> _handleDeepLink(Uri uri) async {
    ...
    try {
      ...
      _router.push(LobbyRoute($extra: call, user: user).location, extra: call);
    } catch (e, stk) {
  ...
  void _onCallAccept(ActionCallAccept event) async {
    ...
    // Assuming this is where the user ID is stored
    final userSnapshot =
        await FirebaseDatabase.instance.ref('users/$uuid').get();
